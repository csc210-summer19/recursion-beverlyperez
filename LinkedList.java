...
public class LinkedList<E extends Comparable<E>> {
	...
	private E get(Node ref, int startIndex, int stopIndex) {
		// unsure of how to increment, n involved?
	...
	public void removeAll(E el) {
		...
		removeHeadHelp(el);
		removeAllHelp(first, el);

	}

	private void removeAllHelp(Node ref, E el) {
		if (ref.next == null) {
			return;
		}
		if (el.equals(ref.next.data)) {
			...
			n--;
			if (ref.next != null) {
				removeAllHelp(ref, el);
			}

		}

		else {
			removeAllHelp(ref.next, el);
		}

	}

//help
	private void removeHeadHelp(E el) {
